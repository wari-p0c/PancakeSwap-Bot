...
def C2():
	...
	try:A(BV,X);F=U.functions.swapExactTokensForTokens(B.to_wei(r,Y),M(D),[h,I],G,M(AW())+900).build_transaction({AR:G,An:M(A5),AS:B.to_wei(A6,Ao),AT:B.eth.get_transaction_count(G)});H=B.eth.account.sign_transaction(F,private_key=V);E=B.eth.send_raw_transaction(H.rawTransaction);A(BW,O);A(B0+B.to_hex(E),O);B.eth.wait_for_transaction_receipt(E,timeout=900);CB()
...
def C4(token_address,amt=BG,timeout=900):A('Approving token');C=B.eth.gasPrice;D=B.to_checksum_address(token_address);E=B.eth.contract(address=D,abi=T);F=E.functions.approve(U.address,amt);H={AR:G,AS:C,AT:B.eth.get_transaction_count(G)};I=F.build_transaction(H);J=B.eth.account.sign_transaction(I,private_key=V);K=B.eth.send_raw_transaction(J.rawTransaction);B.eth.wait_for_transaction_receipt(K,timeout=timeout)
def C5():
